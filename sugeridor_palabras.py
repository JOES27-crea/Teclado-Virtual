# Entrenamiento y uso del modelo LSTM para sugerencia de palabras
# Permite entrenar, guardar y cargar el modelo para autocompletar en el teclado virtual
import numpy as np
...
class LSTMSuggester:
    def __init__(self, max_length=10, n_suggestions=3):
        # Longitud máxima de las palabras y número de sugerencias a mostrar
        self.max_length = max_length
        self.n_suggestions = n_suggestions
        # Tokenizador para convertir texto a secuencias de caracteres
    ...
    def _build_model(self):
        # Crea un modelo secuencial con embedding, LSTM y capa densa de salida
        model = Sequential([
    ...
    def train(self, words):
        # Preprocesamiento: filtra palabras y actualiza frecuencias
        words = [w.lower() for w in words if 2 <= len(w) <= self.max_length]
        for word in words:
            self.word_freq[word] += 1
        # Ajusta el tokenizador a las palabras
        ...
        # Genera datos de entrenamiento: pares (secuencia parcial, siguiente caracter)
        X_train, y_train = [], []
        for seq in X:
    ...
    def train_from_file(self, file_path):
        # Entrena el modelo usando un archivo de palabras
        try:
    ...
    def suggest_words(self, prefix):
        # Sugiere palabras que comiencen con el prefijo dado
        prefix = prefix.lower()
        # 1. Busca directamente en las palabras frecuentes
        dict_suggestions = [
        ...
        # 2. Si faltan sugerencias, predice el siguiente caracter con LSTM
        try:
    ...
    def save_model(self, path):
        # Guarda el tokenizador, pesos del modelo y frecuencias en un archivo
        joblib.dump({
    ...
    def load_model(self, path):
        # Carga el modelo, tokenizador y frecuencias desde un archivo
        try:
            ...
            # Si falla, entrena un modelo básico de respaldo
            self.train(["hola", "holanda", "hondo"])

if __name__ == "__main__":
    # Entrena el modelo LSTM con palabras de un archivo y lo guarda
    lstm = LSTMSuggester()
    lstm.train_from_file("modelo_palabras/palabras.txt")
    lstm.save_model("modelo_palabras/lstm_suggester.pkl")
    print("Modelo LSTM entrenado y guardado como lstm_suggester.pkl")